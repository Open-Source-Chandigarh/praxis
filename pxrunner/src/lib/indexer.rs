pub mod indexer {
    ...
    fn fetch_local_db() -> Result<String, Box<dyn std::error::Error>> {
        ...
        let content =
            fs::read_to_string(path).expect("No db detected! PLease px init in new directory");
        Ok(content)
    }
    ...
    pub fn exercises() -> Result<Vec<Exercise>, std::io::Error> {
        ...
        if !syncdbs() {
            return Err(Error::new(
                ErrorKind::Other,
                "Db's Altered! Please init in a new directory",
            ));
        }
        ...
        allquestions.updatestatus(
            "00_print".to_string(),
            "rust".to_string(),
            "00_intro".to_string(),
            "./00_intro/00_print".to_string(),
            true,
        );
